...
// assert retcode is not an error and return it if not true
#define A76XX_SSL_ASSERT_RETCODE(retcode) {               \
        if( (retcode) != A76XX_SSL_OPERATION_SUCCEEDED) { \
            return retcode;                               \
        }                                                 \
    }

// error codes
#define A76XX_SSL_OPERATION_TIMEDOUT          -2
#define A76XX_SSL_GENERIC_ERROR               -1

// see section 19.3.1 of A76XX Series_AT Command Manual_V1.07
#define A76XX_SSL_OPERATION_SUCCEEDED          0
...
class A76XX_SSL_Commands {
  ...
  public:
    ...
    A76XX_SSL_Commands(A76XX& modem)
        : _modem(modem)
        , _last_error(A76XX_SSL_OPERATION_SUCCEEDED) {}
    ...
    int8_t config_ssl_sslversion(uint8_t ssl_ctx_index, uint8_t ssl_version) {
        ...
        return _modem.waitResponse() == Response_t::A76XX_RESPONSE_OK ? A76XX_SSL_OPERATION_SUCCEEDED : A76XX_SSL_GENERIC_ERROR;
    }
        ...
        switch (rsp) {
            case Response_t::A76XX_RESPONSE_MATCH_1ST : {
                ...
                if (_modem.waitResponse() == Response_t::A76XX_RESPONSE_OK) {
                    return A76XX_SSL_OPERATION_SUCCEEDED;
                } else {
                    return A76XX_SSL_GENERIC_ERROR;
                }
            }
            case Response_t::A76XX_RESPONSE_TIMEOUT : {
                return A76XX_SSL_OPERATION_TIMEDOUT;
            }
            default : {
                return A76XX_SSL_GENERIC_ERROR;
            }
    ...
    int8_t cert_overwrite(const char* cert, const char* certname) {
        ...
        if (cert_exists(certname)) {
            ...
            A76XX_SSL_ASSERT_RETCODE(retcode);
        }
