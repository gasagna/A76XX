...
// use the sketch SaveCertificate to store mosquitto.org certificate to the module
...
void setup() {
    ...
    if (mqtt.setCaCert(MOSQUITTO_CA_CERTNAME) == false) {
        Serial.println("error");
        while (true) {}
    }
    Serial.println("done");
    ...
    if (mqtt.connect(server, port, clean_session, keepalive, 
